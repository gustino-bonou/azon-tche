    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form @submit.prevent="doStoreProject">
            <div class="mb-3">
...
export default {
  ...
  setup(propos, emit) {
    ...
    const { success, errors, storeProject } = projectService();

    const isLoading = ref(false);

    const currentMoment = ref("");
    const doStoreProject = async () => {
      isLoading.value = true;
      await storeProject(form).then(() => {
        if (success.value) {
          resetForm();
          closeModal();
          emit.emit("dataFresh");
    ...
    const closeModal = () => {
      const taskModalElement = document.getElementById("addProjectModal");
